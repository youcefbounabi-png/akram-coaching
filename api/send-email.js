...
export default async function handler(req, res) {
  ...
  try {
    ...
    const {
      type, plan, name, email, whatsapp, age, gender, country,
      weight, height, injuries, goal, message,
    ...
    const attachments = [];
    if (frontPic && frontPic.startsWith('data:image')) {
      const content = frontPic.split(',')[1];
      attachments.push({ filename: 'front_progress.jpg', content });
    }
    if (sidePic && sidePic.startsWith('data:image')) {
      const content = sidePic.split(',')[1];
      attachments.push({ filename: 'side_progress.jpg', content });
    }
    if (backPic && backPic.startsWith('data:image')) {
      const content = backPic.split(',')[1];
      attachments.push({ filename: 'back_progress.jpg', content });
    }
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#050505;padding:40px 20px;">
    <tr><td align="center">
      ...
      row('Country', country),
          ...
          ${isBooking && (date || time) ? section('🗓️ Appointment Details', [
      row('Selected Date', date),
      row('Selected Time', time),
    ].join('')) : ''}

          ${isIntake ? section('🏥 Body & Health', [
      ...
      row('Message', message),
          ...
          <div style="margin-bottom:28px;">
            ...
            <div style="background:#111;padding:16px;border-radius:0 0 8px 8px;text-align:center;">
              <p style="color:#9ca3af;font-size:12px;margin:0;">Photos are attached to this email as files.</p>
            </div>
          </div>` : ''}
    ...
    const result = await resend.emails.send({
      ...
      subject: `🏋️ ${isBooking ? 'Book' : isIntake ? 'Intake' : 'Msg'} — ${name}`,
      html: html,
