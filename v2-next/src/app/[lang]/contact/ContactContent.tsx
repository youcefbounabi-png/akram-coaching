...
import { Send, MessageSquare, MapPin, Calendar, CheckCircle2, Phone } from 'lucide-react';
                                    ...
                                    {selectedDate && (
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="mt-6" dir="ltr">
                                            <div className="grid grid-cols-3 gap-2">
                                                {TIMES.map(time => (
                                                    <button key={time} type="button" onClick={() => setSelectedTime(time)} className={`py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 cursor-pointer ${selectedTime === time ? 'bg-brand-red/20 border-brand-red text-brand-red' : 'bg-white/5 border-white/10 text-white/60 hover:bg-white/10 hover:border-white/20 hover:text-white'}`}>
                                                        {time}
                                                    </button>
                                                ))}
                                            </div>
