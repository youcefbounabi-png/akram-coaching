                        ...
                        {categories.map(cat => (
                            ...
                            >
                                ...
                                <span className="ml-2 text-xs opacity-60">
                                    ({cat.key === 'all' ? transformations.length : transformations.filter(item => item.category === cat.key).length})
                                </span>
                            </button>
                        ))}
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {testimonials.map((item, i) => (
                            <motion.div key={item.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: i * 0.1 }} viewport={{ once: true }} className="glass-panel glass-panel-hover rounded-2xl p-8 cursor-default">
                                ...
                                <p className="text-white/70 text-sm leading-relaxed mb-6 font-light italic text-start">&ldquo;{item.text}&rdquo;</p>
                ...
                <div className="container max-w-3xl mx-auto px-6">
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            <Link href={`/${lang}/pricing`} className="inline-flex items-center justify-center gap-3 bg-brand-red text-white px-10 py-5 rounded-full font-bold text-lg hover:scale-105 active:scale-95 transition-all red-glow-strong group cursor-pointer">
                            ...
                            <Link href={`/${lang}/contact`} className="inline-flex items-center justify-center gap-3 glass-panel glass-panel-hover px-10 py-5 rounded-full font-bold text-lg cursor-pointer">
