      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div
          className={cn(
            ...
            isRTL
              ? '-right-[10%] lg:right-auto lg:left-0 xl:left-[-2.5rem]'
              : '-left-[10%]  lg:left-auto  lg:right-0 xl:right-[-2.5rem]'
          )}
          style={{
          ...
          <div className="mt-16 flex flex-wrap items-center gap-x-12 gap-y-8 border-t border-white/10 pt-10" dir="ltr">
            {BRAND.stats.slice(0, 3).map((stat) => {
              ...
              if (stat.label === "Success Stories" || stat.label === "Success stories") {
                return <Link key={stat.label} href={`/${lang}/transformations`}>{content}</Link>;
              }
              return content;
            })}
          </div>
