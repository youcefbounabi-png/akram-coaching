...
export default function Header() {
  ...
  const navLinks = [
    ...
    { name: t.nav.challenge, href: `/${lang}/challenge` },
    { name: t.nav.transformations, href: `/${lang}/transformations` },
    { name: t.nav.pricing, href: `/${lang}/pricing` },
    { name: t.nav.contact, href: `/${lang}/contact` },
  ];

  const isActive = (href: string) => {
    if (href === `/${lang}`) return pathname === `/${lang}` || pathname === '/';
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link href={`/${lang}`} className="flex items-center gap-3 group" aria-label="Coach Akram Home">
          <img
            src={BRAND.logo}
            alt={BRAND.name}
